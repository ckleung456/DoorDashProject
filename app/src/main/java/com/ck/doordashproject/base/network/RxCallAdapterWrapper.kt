...
class RxCallAdapterWrapper constructor(val retrofit: Retrofit, val wrapped: CallAdapter<Any, Any>?) :
        CallAdapter<Any, Any> {
    ...
    private fun asRetrofitException(throwable: Throwable, type: Type): RetrofitException {
        if (throwable is HttpException) {
            ...
            return RetrofitException.httpError(response.raw().request().url().toString(), response, retrofit, type)
        }
