...
class RestaurantAdapterPresenterImpl(val view: RestaurantAdapterView): RestaurantAdatperPresenter {
    ...
    @VisibleForTesting
    var restaurantsList = ArrayList<RestaurantDataModel>()

    override fun getItemCount(): Int {
        return restaurantsList.size
    }

    override fun setRestaurants(newList: ArrayList<RestaurantDataModel>) {
    ...
    override fun bindDataToHolder(holder: RestaurantViewHolder, position: Int, payloads: MutableList<Any>) {
        val data: RestaurantDataModel
        if (payloads.isEmpty()) {
            data = restaurantsList[position]
        } else {
            val diffBundle = payloads[payloads.size - 1] as Bundle
            data = diffBundle.getParcelable(RestaurantsDiffCallback.EXTRA_DIFF_DATA) as RestaurantDataModel
        }
        holder.onBind(data)
    }
}